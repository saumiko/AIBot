    ...
    public void PaintAndCheck(Graphics2D g){
        for (row = 1; row <= 9; row++) {
            for (column = 1; column <= 13; column++) {
                if(Character[row][column-1]!=0 && Character[row][column+1]!=0  && Player[row][column-1]==1 && Player[row][column+1]==2){
                        g.drawImage(fight, X[row][column], Y[row][column], 70, 70, null);
